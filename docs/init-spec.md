...
- Cybrary (Jeremy Gehring) — building a new AI product, needs to understand guardrails, human checkpoints, and cost profile. The assessment could reactivate the deal and expand scope beyond the $5K RBAC and $7.5K Behavioral Analytics proposals.
- ARMI (Taylor McLeod) — evaluating AI/data infrastructure. A workflow assessment demonstrates FDS value beyond Snowflake.
...
Sales team enriching CRM records with AI-generated firmographic data. Cybrary-relevant: connects to the Salesforce/HubSpot world.

### `examples/sales_outreach_agent.yml`
AI drafting personalized sales emails based on prospect research. Connects to the GTM engine work (Pinecone demo). Tool selection risk is high (which CRM field to update, which email to send).
...
## 11. Cybrary Reactivation Play

The pitch to Jeremy:

"I've been thinking about the AI product work you're ramping up at Cybrary. Before you go deep on implementation, I built an open-source workflow assessment tool that maps AI workflows against failure modes, identifies where humans need to stay in the loop, and calculates total cost at scale — including Snowflake infrastructure costs.

Want me to run it against one of your planned AI workflows? Takes about an hour, I share the full report, and it'll give you a clear picture of where the guardrails and evals need to sit before you ship to users.

The behavioral analytics proposal I sent earlier is one example of a workflow this could assess — happy to use that as the test case."

This connects the tool to the existing $7.5K proposal and positions FDS as the consultancy that helps Cybrary build AI products safely, not just maintain their Snowflake instance.

---

## 12. Relationship to Nate's Seven Skills
